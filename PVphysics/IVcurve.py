...
def setparameters(G, T, Iph_stc, I0_stc, Rs, Rsh_stc, a_stc, b_stc=0, m_stc=0,
                  Vbr_stc=None):
...
def _unpackParam(p):
    return (p['I0'], p['Iph'], p['Rs'], p['Rsh'], p['a'],
            p['b'], p['m'], p['Vbr'])
...
def area_criterion(Vmeas, Imeas, Icalc):
    ...
    al = np.abs((Vmeas[m + 1] - Vmeas[m]) * ((Icalc[m] - Imeas[m + 1])**2 + (
                Icalc[m + 1] - Imeas[m + 2])**2) / (2 * (
                    np.abs(Icalc[m] - Imeas[m + 1]) +
...
def fitting_algorithm(Ipoints, Vpoints):  # , fixed_ideality=0):
    ...
    # 2-D array for input to curve_fit
    I_V_points = np.array([Vpoints, Ipoints])
    # get this to work w/actual data
    # Finds closest point to Isc to use as guess for Iph:
    # np.where(np.abs(Vpoints)==min(np.abs(Vpoints)))
    index1 = np.argmin(np.abs(Vpoints))
    I_zero = Ipoints[index1]
    I_zero = float(I_zero)  # changes to float from array
    index2 = np.argmin(np.abs(Ipoints))
    # both original Izero and Vzero have problems
    V_zero = float(Vpoints[index2])
    AC = 10  # intialise AC
    AC_list = [AC]
    ...
    while (AC > 0.8):
        try:
            ...
            # solves curve for guessed parameters
            I_guess = solvecurve(params, Vpoints)
